...
//这个文件与activityonvertex.c 不一样，这里吧拓扑排序用一个函数实现
//以便于关键路径算法调用
#include<stdio.h>
#include<stdlib.h>
...
#ifndef ACTIVITYONVERTEX_H
#define ACTIVITYONVERTEX_H

static void CountIndegree(OLGraph *olg, int *indegree);    //统计各顶点的入度
static void ShowIndegree(int *indegree, int n);
static void EvalueTopo(OLGraph *olg, int *indegree, int *top); //栈在函数内创建
...
*/
void TopoSeq(OLGraph *olg, int *topo_list)
{
   //OLGraph olg;
   // IntLinkStack zero_degree_stack = NULL;
    int *indegree = NULL;
    ...
    EvalueTopo(olg, indegree, topo_list);
    printf("topo序列如下\n");
    ShowIndegree(topo_list,vexnum);            //显示topo数组内容
...
void EvalueTopo(OLGraph *olg, int *indegree, int *topo)
{
    ...
    if (topo_index != vexnum)
    {
        printf("topo序中有%d个顶点，这不是一个无环图\n",topo_index);
        printf("拓扑序列的内容如下:\n");
        ShowIndegree(topo,topo_index);            //显示topo数组内容
        exit(EXIT_FAILURE);
    }
